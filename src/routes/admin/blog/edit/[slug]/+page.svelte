<script lang="ts">
	...
	let content_md = $state((() => data.post.content_md ?? '')());
...
<div class="blog-edit-page">
	...
	<form method="POST" action="?/save" use:enhance class="blog-form">
		{#if data.returnPath}
		...
		<div class="form-actions">
			...
			{#if data.returnIsList}
				<a href={resolve('/blog')}>Cancel</a>
			{:else if data.returnSlug}
				<a href={resolve('/blog/[slug]', { slug: data.returnSlug })}>Cancel</a>
			{:else}
				<a href={resolve('/blog/[slug]', { slug: data.post.slug })}>Cancel</a>
			{/if}
		</div>
	</form>
</div>
